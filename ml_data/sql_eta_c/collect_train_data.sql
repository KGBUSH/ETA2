...
#define label='指定开始时间',${data_dt1}='2020-03-01';
#define label='指定结束时间',${data_dt2}='2020-03-15';
#define label='进圈距离',${distance}=80;  --学坤订的50米参数
-- distinct order_id's count=51738
drop table algo_test.dy_eta_c_train_01;
create table algo_test.dy_eta_c_train_01 as
select
  A.*,
  B.t_lat,
  B.t_lng,
...
-- count = 287
drop table algo_test.dy_eta_c_train_02;
create table algo_test.dy_eta_c_train_02 as
select
  *
from
  algo_test.dy_eta_c_train_01
where
  row_num = 1
...
--3. 把刚才那些进圈后用时很久的订单剔除
drop table algo_test.dy_eta_c_train_03;
create table algo_test.dy_eta_c_train_03 as
select
  A.*
from
  algo_test.dy_eta_c_train_01 as A
  left outer join algo_test.dy_eta_c_train_02 as Invalid on (
...
from algo_test.dy_eta_c_train_03;



--4 (重要)这里对之前的表做group by，得到交付时间和交付上报条数,
-- 1001-1014的order量是678475
drop table algo_test.dy_eta_c_train_04;
create table algo_test.dy_eta_c_train_04 as
select
  *
from
  (
...
from algo_test.dy_eta_c_train_04;



-- train 部分不需要
-- -- 5* hive 运行，对达达做peek平均交付时间统计, 又加上了各时间阶段阶段  数据库
-- drop table algo_test.dy_eta_c_train_05_peek;
-- create table algo_test.dy_eta_c_train_05_peek AS
-- select
...
--             from
--               algo_test.dy_eta_c_train_04 as tmp
--           ) as tmp1
--       ) a
--     group by
--       transporter_id,
...
--6.  订单绑定到POI信息
drop table algo_test.dy_eta_c_train_06;
create table algo_test.dy_eta_c_train_06 as
select
  T1.*,
  T2.poi_id,
  T2.poi_name,
...
-- drop table algo_test.dy_eta_c_train_07_poi_statistics;
-- create table algo_test.dy_eta_c_train_07_poi_statistics as
-- select
--   row_number() over() as id,
--   a.poi_id,
--   --a.poi_name,
...
--8 计算 POI 级别均值、中位数等服务时效，****放开distance的限制***
-- key 加入 poi的经纬度
-- 加入方差，transporter_id等数据
-- 加入supplier_id，s_r直线距离
drop table algo_test.dy_eta_c_train_08;
create table algo_test.dy_eta_c_train_08 as
select
  A.order_id,
  A.supplier_id,
  A.transporter_id,
...
from
  ...
  join algo_test.dy_eta_c_07_poi_statistics as B on (  -- 上面做的统计表
    A.poi_id = B.poi_id
...
--9. 把transporter的信息合入进来，做到algo_db中, 最后又转存到algo_test
drop table algo_test.dy_eta_c_train_09;
create table algo_test.dy_eta_c_train_09 as
select
  bef.*,
...
from
  algo_test.dy_eta_c_train_08 as bef
...
--10 拼接
drop table algo_test.dy_eta_c_train_10;
create table algo_test.dy_eta_c_train_10 AS
select
  a.*,
  b.delivery_cnt,
  ...
  b.cnt_peek3,
  b.cnt_peek0,
  b.per_delivery_time_peek1,
  b.per_delivery_time_peek2,
  b.per_delivery_time_peek3,
  b.per_delivery_time_peek0
from
  algo_test.dy_eta_c_train_09 as a
  inner join algo_test.dy_eta_c_05_peek as b on (
