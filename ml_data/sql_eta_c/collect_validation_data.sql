...
#define label='指定开始时间',${data_dt1}='2020-03-14';  --两个时间范围：大于等于和小于等于
#define label='指定结束时间',${data_dt2}='2020-03-15';
#define label='进圈距离',${distance}=80;  --学坤订的50米参数
-- distinct order_id's count=51738
drop table algo_test.dy_eta_c_vali_01;
create table algo_test.dy_eta_c_vali_01 as
select
  A.*,
  B.t_lat,
  B.t_lng,
...
-- count = 287
drop table algo_test.dy_eta_c_vali_02;
create table algo_test.dy_eta_c_vali_02 as
select
  *
from
  algo_test.dy_eta_c_vali_01
where
  row_num = 1
...
--3. 把刚才那些进圈后用时很久的订单剔除
drop table algo_test.dy_eta_c_vali_03;
create table algo_test.dy_eta_c_vali_03 as
select
  A.*
from
  algo_test.dy_eta_c_vali_01 as A
  left outer join algo_test.dy_eta_c_vali_02 as Invalid on (
...
from algo_test.dy_eta_c_vali_03;



--4 (重要)这里对之前的表做group by，得到交付时间和交付上报条数,
-- 1001-1014的order量是678475
drop table algo_test.dy_eta_c_vali_04;
create table algo_test.dy_eta_c_vali_04 as
select
  A.order_id,
  A.receiver_id,
  A.supplier_id,
  ...
  A.cargo_weight
from
  algo_test.dy_eta_c_vali_03 as A
group by
  A.order_id,
  A.receiver_id,
  A.supplier_id,
...
--5.  订单绑定到POI信息
drop table algo_test.dy_eta_c_vali_05;
create table algo_test.dy_eta_c_vali_05 as
select
  T1.*,
  T2.poi_id,
  T2.poi_name,
...
--6 计算 POI 级别均值、中位数等服务时效，****放开distance的限制***
-- key 加入 poi的经纬度
-- 加入方差，transporter_id等数据
-- 加入supplier_id，s_r直线距离
drop table algo_test.dy_eta_c_vali_06;
create table algo_test.dy_eta_c_vali_06 as
select
  A.order_id,
  A.supplier_id,
  A.transporter_id,
  ...
  B.percentile_deliver_time_poi,  --下面三个都是poi的历史数据
  ...
  A.deliver_time1,
  A.deliver_time2,
  abs(B.percentile_deliver_time_poi - A.deliver_time1) as error1,  -- POI中位统计时间
  abs(B.percentile_deliver_time_poi - A.deliver_time2) as error2
from
  (
    select
      a.*
    from
      algo_test.dy_eta_c_vali_05 a
  ) A
  join (
    select
      a.poi_id,
      a.poi_name,
      percentile(int(a.deliver_time1), 0.5) as percentile_deliver_time_poi,  -- 这个楼的历史平均交付时间，下面会做达达的交付时间
      ...
      stddev(a.deliver_time1) as std_delivery_time_poi,  --加入方差
      -- 这个poi周围的收货人的平均收货时间
...
-- 7* api_algo_poi_statistic_Info  poi统计信息，数据库要用
...
-- 9 presto 运行，对 7中的达达做平均交付时间统计, 又加上了各时间阶段阶段
drop table algo_test.dy_eta_c_09;
create table algo_test.dy_eta_c_09 AS
select
  a.transporter_id,
  a.city_id,
...
-- 10 上面数据要加id time才能进线上的dw_api  已经移到了algo_test
drop table algo_test.dy_eta_c_10_peek;
create table algo_test.dy_eta_c_10_peek as
select
  row_number() over() as id,
  -- 自增
  a.*,
...
from
  algo_test.dy_eta_c_09 as a



--11 presto 运行【重要】7 和7.1拼起来 变成了一个有天气，有geohash，有骑手平均交付时间
drop table algo_test.dy_eta_c_11;
create table algo_test.dy_eta_c_11 AS
select
  a.*,
  b.delivery_cnt,
...
from
  ...
  inner join algo_test.dy_eta_c_10_peek as b on (
